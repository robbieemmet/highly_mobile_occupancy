...
```{r explore_sim_data_dt_data_loss}
#library(jagsUI)
#library(ggplot2)
...
sim_data_summaries <- data.frame(stringsAsFactors=FALSE)
...
for(detectprob in c(0.2,0.5, 0.8)){
  for(occ in c(0.2, 0.5,0.8)){
    for(q in c(0.2, 0.5,0.8)){
      ...
      for(reps in 1:nsimdata){
        data_in <- load(paste(scen_folder, "/rep", reps, "/simdata.Rdata", sep = ""))
        #temp_data <- data_in
        ct_data <- temp_data$ctdo_in
        dt_data <- temp_data$do_in
        
        ndetects_ct <- sum(ct_data$y)
        ndetects_dt <- sum(dt_data$x)
        ndetoccasions_ct <- sum(ct_data$y>0)
        ndetoccasions_dt <- 0 # add something here?
        dt_data_sum <- apply(dt_data$x, c(1,2),sum)
        mismatch_data <- ifelse(any(ct_data$y>0 & dt_data_sum==0),1,0)
        # variable to see, for each ct occasion with detections, how many dt tertiary occasions have detections??
        
        sim_data_summaries <- rbind(sim_data_summaries, c(detectprob, occ, q, reps, ndetects_ct, ndetects_dt, ndetoccasions_ct, mismatch_data))
      }
    }
  }
}
...
# Summaries
names(sim_data_summaries) <- c("det", "occ", "use", "reps", "ndets_ct", "ndets_dt", "noccsdet_ct", "mismatch_data")

tapply(sim_data_summaries$ndets_ct, list(sim_data_summaries$det, sim_data_summaries$occ, sim_data_summaries$use), mean)

tapply(sim_data_summaries$ndets_dt, list(sim_data_summaries$det, sim_data_summaries$occ, sim_data_summaries$use), mean)
...
# Extra analysis - spatial extent of cameras and average distance between them
wa.cams.unique <- wa.cams %>% distinct(LocationName, .keep_all=TRUE)
wa.cams.sp <- SpatialPoints(coords=cbind(wa.cams.unique$UTM_E, wa.cams.unique$UTM_N))
proj4string(wa.cams.sp) <- "+proj=utm +zone=10 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0"

((max(wa.cams.sp@coords[,1])-min(wa.cams.sp@coords[,1]))*(max(wa.cams.sp@coords[,2])-min(wa.cams.sp@coords[,2])))/1000000 # doesn't seem right
wa.cam.dists <- pointDistance(wa.cams.sp, lonlat = FALSE)/1000
wa.cam.dists <- as.matrix(wa.cam.dists)
mean(wa.cam.dists[upper.tri(wa.cam.dists)])

CoordsForRobbie_201130 <- read_excel("C:/Users/Robbie/Downloads/CoordsForRobbie_201130.xlsx")

CoordsForRobbie_201130$Lat <- CoordsForRobbie_201130$`Lat-DD`
CoordsForRobbie_201130$Long <- CoordsForRobbie_201130$`Long-DD`
wa.cams.lonlat <- SpatialPoints(coords=cbind(CoordsForRobbie_201130$Long, CoordsForRobbie_201130$Lat))
wa.cams.lonlat.dist <- pointDistance(wa.cams.lonlat, lonlat=TRUE)/1000
wa.cams.lonlat.dist <- as.matrix(wa.cams.lonlat.dist)
mean(wa.cams.lonlat.dist[lower.tri(wa.cams.lonlat.dist)])
wa.bbox.pts <- data.frame(expand.grid(wa.cams.lonlat@bbox[1,], wa.cams.lonlat@bbox[2,]))
wa.bbox.pts <- wa.bbox.pts[c(1,2,4,3),]
areaPolygon(wa.bbox.pts)/1000000
#library(spatstat)
#area(boundingbox(wa.cams.lonlat))
#wa.cams.area <- areaPolygon(wa.cams.lonlat@bbox)
...
The data that support the findings of portions of this study are available from the Washington Department of Fish and Wildlife. Restrictions apply to the availability of these data, which were used under license for this study. Please contact the department directly with data requests.
...
ggplot(results_for_plotting2, aes(x = model2, y = occprob)) + 
  geom_boxplot(aes(fill = factor(trueuse))) + 
  facet_grid(detection ~ occupancy, labeller = label_both) +
  geom_hline(data = occupancy_lines, aes(yintercept = occline), colour = "black") + 
  scale_fill_grey(start=0,end=.9) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.x = element_text(angle = 45, hjust=1), text=element_text(size=15)) + 
...
*Web Figure 1. Posterior mean estimates of detection probability from all simulation scenarios where true per-visit detection probability was 0.2. True occupancy probability increases from the left panel (probability 0.2) to the right panel (probability 0.8). The black, dark gray, and light gray boxplots represent scenarios with true use probability of 0.2, 0.5, and 0.8, respectively. *
```{r simulation_fig_occ, results = "asis"}
occupancy_lines <- data.frame(occline = c(0.2,0.5,0.8,0.2,0.5,0.8,0.2,0.5,0.8), occupancy = c(0.2,0.5,0.8,0.2,0.5,0.8,0.2,0.5,0.8))
...
ggplot(results_for_plotting2[results_for_plotting2$detection==0.2,], aes(x = model2, y = use)) + 
  geom_boxplot(aes(fill = factor(trueuse))) + 
  facet_grid(detection ~ occupancy, labeller = label_both) +
  geom_hline(yintercept = c(0.2,0.5,0.8), colour = "black") + 
  scale_fill_grey(start=0,end=.9) +
  scale_y_continuous(breaks=c(0.2,0.5,0.8))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), axis.text.x = element_text(angle = 45, hjust=1), text=element_text(size=15)) + 
...
, tapply(pwr_all_reps$trend_occuse, list(pwr_all_reps$R, pwr_all_reps$pcells, pwr_all_reps$modtype), mean)
), parameter = c(rep("occupancy", 24), rep("inst. occupancy", 24)))
